...
try:
    ...
    # logger is defined below; avoid using it before definition
    # Logging will be done after logger is created

# Ensure logger is configured (in case main didn't)
...
class AppGUI(QMainWindow):
    def __init__(self, db_manager, config, repository=None, settings=None):
        ...
        self.repository = repository  # Repository for Firestore/SQLite abstraction
        self.settings = settings      # AppSettings instance

        logger.debug(f"AppGUI recibe config: {self.config}")
        ...
        self.setWindowTitle(
            f"Gestor de Alquileres - Fuente de datos: {data_source_display}"
        )
        ...
        # 3. Cargar configuración (legacy)
        try:
            legacy_config = config_manager.cargar_configuracion()
            if legacy_config:
                self.config.update(legacy_config)
        except Exception as e:
            logger.warning("No se pudo cargar configuración legacy: %s", e)
...
# Registro de Alquileres
        try:
            self.registro_tab = RegistroAlquileresTab(
                self.db, self.proyecto_actual, config=self.config
            )
            self.tabs.addTab(self.registro_tab, "Registro de Alquileres")
        # ... (el resto del try/except sigue igual) ...
        except Exception as e:
            logger.error("Error creando tab Registro de Alquileres: %s", e)
            placeholder = QWidget()
            layout = QVBoxLayout(placeholder)
            layout.addWidget(
                QLabel(
                    "⚠️ Error cargando pestaña Registro de Alquileres:\n"
        ...
        try:
            ...
            logger.error("Error creando tab Gastos Equipos: %s", e)
            placeholder = QWidget()
            layout = QVBoxLayout(placeholder)
            layout.addWidget(
                QLabel(
                    "⚠️ Error cargando pestaña Gastos Equipos:\n"
                    f"{str(e)}\n\nIntenta migrar la base de datos completa."
                )
            )
        ...
        try:
            ...
            logger.error("Error creando tab Pagos a Operadores: %s", e)
            placeholder = QWidget()
            layout = QVBoxLayout(placeholder)
            layout.addWidget(
                QLabel(
                    "⚠️ Error cargando pestaña Pagos a Operadores:\n"
                    f"{str(e)}\n\nIntenta migrar la base de datos completa."
                )
            )
        ...
        try:
            ...
            logger.error("Error creando tab Dashboard: %s", e)
            placeholder = QWidget()
            layout = QVBoxLayout(placeholder)
            layout.addWidget(
                QLabel(
                    "⚠️ Error cargando pestaña Dashboard:\n"
                    f"{str(e)}\n\nIntenta migrar la base de datos completa."
                )
            )
    ...
    def _create_menu_bar(self):
        ...
        archivo_menu.addAction(
            "Restaurar desde Copia de Seguridad...", self._restaurar_backup
        )
        ...
        gestion_menu.addAction(
            "Clientes", lambda: self._abrir_ventana_gestion("Cliente")
        )
        gestion_menu.addAction(
            "Operadores", lambda: self._abrir_ventana_gestion("Operador")
        )
        gestion_menu.addAction("Equipos", self._abrir_ventana_gestion_equipos)
        gestion_menu.addAction(
            "Mantenimiento de Equipos", self._abrir_ventana_mantenimiento
        )
        gestion_menu.addSeparator()
        gestion_menu.addAction("Gestionar Abonos", self._abrir_ventana_gestion_abonos)

        # Herramientas (Firestore / migraciones)
        herramientas_menu = menubar.addMenu("Herramientas")
        herramientas_menu.addAction(
            "Migrar desde SQLite a Firestore...", self._abrir_dialogo_migracion
        )
        herramientas_menu.addAction(
            "Migrar desde Firestore a SQLite...", self._abrir_dialogo_backup
        )
        herramientas_menu.addSeparator()
        herramientas_menu.addAction(
            "Verificar Conexión Firestore", self._verificar_conexion_firestore
        )

        # Configuración
        config_menu = menubar.addMenu("Configuración")
        config_menu.addAction(
            "Seleccionar Carpeta CONDUCES", self.seleccionar_carpeta_conduces
        )
        config_menu.addSeparator()
        config_menu.addAction(
            "Configurar Fuente de Datos (Firestore/SQLite)...",
            self._configurar_fuente_datos,
        )
        config_menu.addAction(
            "Configurar Carpeta de Backups...",
            self._configurar_carpeta_backups,
        )
        ...
        if THEME_MANAGER_AVAILABLE:
            ...
            logger.warning(
                "Theme manager not available - theme menu will be disabled"
            )

    # ------------------------------------------------------------------
    # Selección de base de datos y carpetas
    # ------------------------------------------------------------------
    def elegir_base_datos(self):
        ...
        if archivo:
            ...
            QMessageBox.information(
                self, "Configuración", f"¡Ruta guardada!\n{archivo}"
            )
            self.db = DatabaseManager(archivo)
            self.cargar_proyecto_inicial()
    ...
    def seleccionar_carpeta_conduces(self):
        ...
        nueva_carpeta = QFileDialog.getExistingDirectory(
            self,
            "Seleccionar carpeta base para CONDUCES",
            carpeta_actual or "",
        )
        if nueva_carpeta:
            ...
            QMessageBox.information(
                ...
                f"Carpeta de CONDUCES establecida:\n{nueva_carpeta}",
            )
    ...
    def cargar_proyecto_inicial(self):
        if not self.db:
            QMessageBox.critical(
                self,
                "Error Base de Datos",
                "No hay conexión de base de datos inicializada.",
            )
            return
        ...
        if not proyectos:
            QMessageBox.critical(
                self, "Error Base de Datos", "No se encontraron proyectos."
            )
            return

        proyecto_target = next(
            (p for p in proyectos if p["nombre"] == "EQUIPOS PESADOS ZOEC"), None
        )
        if proyecto_target:
            ...
            data_source_display = self.config.get("data_source_display", "sqlite")
            self.setWindowTitle(
                f"Gestor de Alquileres - {self.proyecto_actual['nombre']} "
                f"[{data_source_display}]"
            )

            # Tabs nuevos
            if self.gastos_equipos_tab:
            ...
            QMessageBox.critical(
                self,
                "Error de Proyecto",
                "No se encontró el proyecto 'EQUIPOS PESADOS ZOEC'.",
            )
    ...
    def cargar_proyecto(self, proyecto_id):
        ...
        if not self.proyecto_actual:
            QMessageBox.critical(
                self, "Error", f"No se encontró el proyecto con ID {proyecto_id}."
            )
            return
    ...
    def _crear_backup(self):
        ...
        if not db_path:
            QMessageBox.warning(
                self, "Advertencia", "No hay base de datos seleccionada."
            )
            return
        ...
        ruta_destino, _ = QFileDialog.getSaveFileName(
            self,
            "Guardar copia de seguridad como...",
            nombre_sugerido,
        ...
        if ruta_destino:
            ...
            QMessageBox.information(
                self, "Éxito", f"Copia de seguridad creada en:\n{ruta_destino}"
            )
    ...
    def _restaurar_backup(self):
        ...
        if not db_path:
            ...
            return
        ruta_backup, _ = QFileDialog.getOpenFileName(
            self,
        ...
        if ruta_backup:
            ...
            QMessageBox.information(
                self,
                "Éxito",
                "Base de datos restaurada correctamente. Reinicia la aplicación.",
            )
    ...
    def generar_reporte_detallado_pdf(self):
        if not self.proyecto_actual:
            QMessageBox.warning(
                self,
                "Proyecto no cargado",
                "Debe cargar un proyecto para generar reportes.",
            )
            return
        ...
        cliente_nombre = (
            dialog.combo_cliente.currentText()
            if cliente_idx != 0
            else "TODOS LOS CLIENTES"
        )
        formato = dialog.formato  # 'pdf' o 'excel'
        moneda = (
            self.proyecto_actual["moneda"]
            if "moneda" in self.proyecto_actual
            else "RD$"
        )

        ext = "pdf" if formato == "pdf" else "xlsx"
        nombre_archivo = (
            f"Reporte_Detallado_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{ext}"
        )
        ...
        if ok:
            QMessageBox.information(
                self, "Éxito", f"Reporte generado en:\n{resultado}"
            )
        else:
            QMessageBox.warning(
                self, "Error al generar reporte", str(resultado)
            )
    ...
    def generar_reporte_operadores(self):
        ...
        nombre_archivo = (
            f"Reporte_Operadores_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{formato}"
        )
    ...
    # ------------------------------------------------------------------
    # Estado de cuenta cliente / general
    # ------------------------------------------------------------------
    def generar_estado_cuenta_cliente_pdf(self):
        ...
        if not filtros:
            QMessageBox.warning(
                self, "Cliente Requerido", "Por favor, seleccione un cliente."
            )
            return
        ...
        if filtros["cliente_id"] is None:
            ...
            # Aquí armamos el título completo que verá el PDF
        ...
        if not facturas:
            QMessageBox.information(
                self,
                "Sin datos",
                "No hay datos para el período o filtros seleccionados.",
            )
            return
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Guardar PDF", nombre_archivo, "PDF (*.pdf)"
        )
        if not file_path:
            return
        ...
        rg = ReportGenerator(
            data=facturas,
            title=title,                       # <-- aquí va el título completo
    ...
    def generar_estado_cuenta_general_pdf(self):
        ...
        if filtros["cliente_id"] is not None:
            QMessageBox.warning(
                self,
                "Error",
                "Este botón es solo para el reporte general de todos los clientes.",
            )
            return
        ...
        if not facturas:
            QMessageBox.information(
                self, "Sin datos", "No hay facturas en el período seleccionado."
            )
            return
        ...
        if ok:
            QMessageBox.information(
                self, "Éxito", f"Reporte general generado en:\n{resultado}"
            )
        else:
            QMessageBox.critical(
                self, "Error", f"No se pudo generar el reporte:\n{resultado}"
    ...
    def _abrir_dialogo_migracion(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"No se pudo importar DialogoMigracionFirestore:\n{e}",
            )
            return
    ...
    def _abrir_dialogo_backup(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"No se pudo importar DialogoBackupSQLite:\n{e}",
            )
            return
    ...
    def _configurar_fuente_datos(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"No se pudo importar DataSourceWidget:\n{e}",
            )
            return
    ...
    def _create_theme_submenu(self, parent_menu):
        ...
        try:
            ...
            for theme_info in themes:
                ...
                action.setChecked(theme_info.id == current_theme)
                action.triggered.connect(
                    lambda checked, tid=theme_info.id: self._apply_theme(tid)
                )
                parent_menu.addAction(action)
        except Exception as e:
            logger.exception("Error creating theme submenu: %s", e)
            error_action = QAction("Error cargando temas", self)
            error_action.setEnabled(False)
            parent_menu.addAction(error_action)
    ...
    def _apply_theme(self, theme_id: str):
        ...
        try:
            ...
            logger.exception("Error applying theme %s: %s", theme_id, e)
            QMessageBox.critical(
                self,
                "Error",
        ...
        try:
            ...
            for action in menubar.actions():
                if action.text() == "Apariencia":
                    ...
                    if menu:
                        current_theme = theme_manager.current_theme_id()
                        theme_list = theme_manager.list_themes()
                        for menu_action in menu.actions():
                            for theme_info in theme_list:
                                if menu_action.text() == theme_info.title:
                                    menu_action.setChecked(
                                        theme_info.id == current_theme
                                    )
                                    break
                    break
        except Exception as e:
            logger.exception("Error refreshing theme menu: %s", e)


# ----------------------------------------------------------------------
# Utilidad para guardar archivos con la extensión correcta
# ----------------------------------------------------------------------
def get_save_file_with_extension(parent, formato, nombre_archivo_sugerido):
    """
    Abre un QFileDialog para guardar archivo, ajustando automáticamente la
    extensión según el filtro. Si el usuario borra la extensión o cambia
    el tipo, se la repone.
    """
    if formato == "pdf":
